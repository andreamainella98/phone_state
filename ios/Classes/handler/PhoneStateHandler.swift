...
class PhoneStateHandler: NSObject, FlutterStreamHandler, CXCallObserverDelegate {
    ...
    public func callObserver(_ callObserver: CXCallObserver, callChanged call: CXCall) {
        ...
        switch status {
        case.CALL_INCOMING, // Incoming call ringing — reset duration timer
            .CALL_OUTGOING: // Outgoing dialing — reset timer (will start once connected)
            resetCallDuration()
        case.CALL_STARTED: // Call connected (incoming answered or outgoing connected)
            if callStartTime == nil {
                startDurationTimer()
            }
        case.CALL_ENDED: // Call finished
            stopDurationTimer()
        default: break
        }
    ...
    public func onListen(withArguments arguments: Any?, eventSink events: @escaping FlutterEventSink) -> FlutterError? {
        ...
        for call in callObserver.calls {
            ...
            if callStatus != .NOTHING {
                ...
                switch callStatus {
                case.CALL_INCOMING, .CALL_OUTGOING: // Reset duration timer for new incoming/outgoing calls
                    resetCallDuration()
